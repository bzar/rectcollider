...
Tile::Tile(float x, float y, float w, float h, int z, glhckTexture* texture, glhckRect const& textureTransform, ew::State* state) : ew::Renderable(),
  o(nullptr), x(x), y(y), w(w), h(h), state(state), roles(this, state)
{
  ...
  glhckObjectPositionf(o, x + w/2, y + h/2, z);
}
